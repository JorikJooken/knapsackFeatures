// compile as "gcc bouknap.c -o bouknapExecutable -lm"
...
typedef long          mtype;   /* number of each item                 */
typedef long          stype;   /* sum of pofit or weight              */
typedef __float128        ptype;   /* product type (sufficient precision) */
...
stype bouknap(int n, int *p, int *w, int *m, int *x, long c)
{
  allinfo a;
  itype y;
  item *tab;
...
main()
{
    FILE *in_file  = fopen("bouKnapTest1.in", "r"); // read only 
    ...
    {
        int ignore;
        fscanf(in_file,"%d",&ignore);
        int pr, we, am;
        fscanf(in_file, "%d %d %d", &pr, &we, &am);
        p[i]=pr;
        w[i]=we;
        m[i]=am;
    }
